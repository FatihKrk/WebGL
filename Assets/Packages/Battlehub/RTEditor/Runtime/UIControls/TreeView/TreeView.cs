    ...
    public class TreeView : ItemsControl<TreeViewItemDataBindingArgs>
    {
        ...
        public void Expand(TreeViewItem item)
        {
            ...
            if (ItemExpanding != null)
            {
                ...
                if (item.CanExpand)
                {
                    StartCoroutine(ExpandAllChildrenCoroutine(item, children, containerIndex, itemIndex));
                    UpdateSelectedItemIndex();
                }
            }
        }

        private IEnumerator ExpandAllChildrenCoroutine(TreeViewItem parentItem, IEnumerable children, int initialContainerIndex, int initialItemIndex)
        {
            int containerIndex = initialContainerIndex;
            int itemIndex = initialItemIndex;
            int batchSize = 100; // Her adımda 50 çocuk oluştur
            int counter = 0;
            ...
            foreach (object childItem in children)
            {
                ...
                childContainer.Parent = parentItem;

                DataBindItem(childItem, childContainer);

                // 50 çocuk oluşturduktan sonra bir çerçeve bekle
                if (counter >= batchSize)
                {
                    counter = 0; // Sayacı sıfırla
                    yield return null; // Bir çerçeve bekle
                }
            }
            canFocus = true;
        }
